    ...
    <div className={`fixed inset-y-0 left-0 transform ${isOpen ? "translate-x-0" : "-translate-x-full"
      ...
      <div className="h-full bg-white border-r border-gray-100 flex flex-col">
        <div className="p-6">
          ...
          <nav className="space-y-1">
            {menuItems.map((item, index) => (
              <button
                key={index}
                className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-colors ${item.active
        ...
        <div className="mt-auto p-6 border-t border-gray-100">
          ...
          <button onClick={handleSignOut} className="mt-6 w-full flex items-center justify-center space-x-2 px-4 py-2 border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition-colors">
            <LogOut size={18} />
            <span>Sign Out</span>
          </button>
        </div>
